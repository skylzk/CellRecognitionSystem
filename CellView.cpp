...
void CCellView::MarkIt(int i, int j)//判断是否是中心点，通过访问标志位遍历该点周围来确定
{
...
void CCellView::SaveIt(int i, int j, int radius)//若是中心点 则保存该点
{
	if (i<1 || i>g_nMapWidth-2 || j<1 || j>g_nMapHeight-2)
	{
		return;
...
void CCellView::OnProcSmooth()   //图像平滑3×3
{
	int wd,ht;
	if(g_hBitmap)
	{
...
void CCellView::OnProcHistogram() //用直方图法获得阈值
{
	if(g_hBitmap)
	{
		CHistogramDlg	hDlg;
		...
		for (wd=0;wd<g_nMapWidth;wd++)
		{
			if (!cur_flag->marked && !cur_flag->visited)
			{
				...
				if (g_nCellTotArea<MAX_HOLE && g_nCellTotArea>0)
					FillTheHole(wd,ht);
//				char msg[128];
//				sprintf(msg,"%d",g_nCellTotArea);   //调试代码
//				InvalidateRect(0,TRUE);
//				MessageBox(msg);
			}
			cur_flag++;
				...
				if (y>0 && y<g_nMapHeight-1 && x>0 && x<g_nMapWidth-1) // 最边上的不用处理
				{
					if (cur_flag->center)
					{
						...
						else
						{
							...
							tot_x=0;
							tot_y=0;
							CalcCenterArea(x,y);    //调用函数计算中心点的面积
							pt.x=tot_x/tot_area;
							pt.y=tot_y/tot_area;
...
void CCellView::CalcCenterArea(int i, int j)//对点（X,Y）的中心区域进行计算
{
	if (i<1 || i>g_nMapWidth-2 || j<1 || j>g_nMapHeight-2)
	{
		return;
...
void CCellView::CountSeeds()
{
	...
	for (ht=0;ht<g_nMapHeight;ht++)
		for (wd=0;wd<g_nMapWidth;wd++)
		{
			...
			{
				g_nCellCount++;
				ProcessCountSeeds(wd,ht,cur_flag);   //调用处理函数
			}
			cur_flag++;
		}
